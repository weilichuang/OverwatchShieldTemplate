Shader "Lexdev/CaseStudies/OverwatchShield"
{
	Properties
	{
		...
		_MainTex("Hex Texture", 2D) = "white" {} //合并了Hex Pulse Texture,Hex Edge Texture,Edge Texture三张纹理，分别放在rgb中
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float4 pulseTex =  tex.g;//tex2D(_PulseTex,i.uv);
				...
				fixed4 hexEdgeTex = tex.r;//tex2D(_HexEdgeTex, i.uv);
				...
				//Outer edge logic
				fixed4 edgeTex = tex.b;//tex2D(_EdgeTex, i.uv);
